...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [BASE] = LAYOUT( /* Qwerty */
    ...
    CKC_A,   CKC_S,   CKC_D,   CKC_F,   KC_G,                       KC_H,    CKC_J,   CKC_K,   CKC_L,   CKC_QUOT ,
    KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_NO,   KC_NO,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH ,
    KC_NO,   KC_NO,   KC_NO,   CKC_ESC, CKC_SPC, CKC_TAB, CKC_ENTER,CKC_BKSP,CKC_DEL, KC_NO,   KC_NO,   KC_NO),

  [NAV] = LAYOUT( 
    QK_BOOT, KC_NO,   KC_NO,   KC_NO,  KC_NO,                  LCTL(KC_Y), LCTL(KC_V), LCTL(KC_C), LCTL(KC_X), LCTL(KC_Z),
...
void on_smtd_action(uint16_t keycode, smtd_action action, uint8_t tap_count) {
    switch(keycode) {
        ...
        SMTD_MT(CKC_QUOT, KC_QUOT, KC_LEFT_GUI)
        SMTD_LT(CKC_ESC, KC_ESC, MEDIA)
        SMTD_LT(CKC_SPC, KC_SPC, NAV)
        SMTD_LT(CKC_TAB, KC_TAB, MOUSE)
        SMTD_LT(CKC_ENTER, KC_ENTER, SYM)
        SMTD_LT(CKC_BKSP, KC_BSPC, NUM)
        SMTD_LT(CKC_DEL, KC_DEL, FUN)
    }
}
